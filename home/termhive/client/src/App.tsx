...
export default function App() {
  ...
  const [mainTab, setMainTab] = useState<MainTab>('wiki');
  ...
  const [layout, setLayout] = useState<GridLayout>(() => {
    const saved = localStorage.getItem('termhive:layout-v2') as GridLayout | null;
  ...
  const [newAgentCli, setNewAgentCli] = useState<Agent['cli']>('shell');
  ...
  useEffect(() => {
    localStorage.setItem('termhive:layout-v2', layout);
  }, [layout]);

  // WebSocket
  const { send, wsRef, state: wsState } = useWebSocket((msg: { type?: string; agentId?: string; status?: string }) => {
  ...
  useEffect(() => {
    if (!selectedProjectId && projects.length > 0) {
      const saved = localStorage.getItem('termhive:selected-project');
      const next = projects.find((project) => project.id === saved) || projects[0];
      setSelectedProjectId(next.id);
    }
  }, [projects, selectedProjectId]);

  useEffect(() => {
    if (selectedProjectId) localStorage.setItem('termhive:selected-project', selectedProjectId);
  }, [selectedProjectId]);
  ...
  const selectedAgent = projectAgents.find((agent) => agent.id === selectedAgentId) || null;
  ...
  useEffect(() => {
    ...
    if (!selectedAgentId || !projectAgents.some((agent) => agent.id === selectedAgentId)) {
  ...
  const openNewTerminal = (cli: Agent['cli']) => {
    ...
    <div className={appCls.join(' ')} style={{ '--sidebar-w': sidebarW + 'px' } as React.CSSProperties}>
      <header className="header">
        ...
        <div className="header-r">
          ...
          />
          <button className="hbtn" title="Command snippets" onClick={() => setSnippetsOpen(true)}>
            <Ic.bolt size={13} />
          </button>
          ...
          <button className="hbtn desktop-only" title="Sign out" onClick={handleLogout}>
            <Ic.user size={13} />
          </button>
          {selectedProject && (
      ...
      <section className="gr">
        {selectedProjectId ? (
          <>
            <div className="gr-subbar">
              ...
              <div className="gr-subbar-r">
                ...
                <button className="batch-btn primary" onClick={() => openNewTerminal('shell')}>
          ...
          <div className="panel-empty" style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16 }}>
            <div>{projectLoadError ? `Could not load projects: ${projectLoadError}` : 'Select or create a project to get started'}</div>
            {projectLoadError && (
              <button className="batch-btn" onClick={() => loadProjects()}>
                <Ic.restart size={11} /> Retry
              </button>
            )}
      ...
      <footer className="st">
        ...
        <div className="st-r">
          ...
          <span
            className={'st-item' + (wsState === 'connected' ? ' ok' : '')}
            style={wsState === 'connected' ? undefined : { color: 'var(--attn)' }}
            title={wsState === 'connected'
              ? 'Live connection active'
              : 'Connection interrupted; TermHive is retrying automatically'}
          >
            ws · {wsState}
          </span>
        </div>
      ...
      <nav className="mobile-dock mobile-only" aria-label="Quick actions">
        <button onClick={() => setMobileMenuOpen(true)}>
          <Ic.menu size={17} /><span>Projects</span>
        </button>
        <button onClick={() => setSnippetsOpen(true)}>
          <Ic.bolt size={17} /><span>Snippets</span>
        </button>
        <button className="primary" onClick={() => openNewTerminal('shell')}>
          <Ic.terminal size={18} /><span>Shell</span>
        </button>
        <button onClick={() => openNewTerminal('ssh')}>
          <Ic.arrowR size={17} /><span>SSH</span>
        </button>
        <button onClick={() => setCommandOpen(true)}>
          <Ic.logo size={17} /><span>Keeper</span>
        </button>
        <button onClick={() => setSettingsOpen(true)}>
          <Ic.settings size={17} /><span>Settings</span>
        </button>
      </nav>
