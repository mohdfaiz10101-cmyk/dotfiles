...
import { WS_MESSAGE_EVENT, WS_OPEN_EVENT, type WSMessage } from '../hooks/useWebSocket';
...
export default function Terminal({ agentId, send, wsRef, onFocus, focused }: Props) {
  ...
  useEffect(() => {
    ...
    const handler = (event: Event) => {
      const msg = (event as CustomEvent<WSMessage>).detail;
      if (msg.type === 'terminal:output' && msg.agentId === agentId && msg.data) {
        term.write(msg.data);
        if (initialLoad) {
    ...
    const handleReconnect = () => {
      send({ type: 'terminal:attach', agentId });
      send({ type: 'terminal:resize', agentId, cols: term.cols, rows: term.rows });
    };
    window.addEventListener(WS_MESSAGE_EVENT, handler);
    window.addEventListener(WS_OPEN_EVENT, handleReconnect);
    ...
    return () => {
      ...
      window.removeEventListener(WS_MESSAGE_EVENT, handler);
      window.removeEventListener(WS_OPEN_EVENT, handleReconnect);
