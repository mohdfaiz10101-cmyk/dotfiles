...
import { WS_MESSAGE_EVENT, type WSMessage } from '../hooks/useWebSocket';
...
export default function CommandPanel({ open, onClose, wsRef, sttCfg }: Props) {
  ...
  useEffect(() => {
    ...
    const handler = (ev: Event) => {
      const data = (ev as CustomEvent<WSMessage & { payload?: BrainEvent }>).detail;
      if (data.type !== 'brain:event' || !data.payload) return;
    ...
    return () => { cancelled = true; window.removeEventListener(WS_MESSAGE_EVENT, handler); };
