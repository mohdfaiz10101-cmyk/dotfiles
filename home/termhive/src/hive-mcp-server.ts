      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            cli: {
              type: 'string',
              description: 'Terminal type: claude, codex, gemini, opencode, shell, or ssh.',
            },
            role: {
              type: 'string',
            ...
            host: { type: 'string', description: 'SSH host; required when cli is ssh.' },
            user: { type: 'string', description: 'SSH user; required when cli is ssh.' },
            port: { type: 'number', description: 'SSH port; defaults to 22.' },
            identityFile: { type: 'string', description: 'SSH identity file, e.g. ~/.ssh/id_ed25519.' },
            startupCommand: { type: 'string', description: 'Optional command to run after opening shell/SSH.' },
          },
        ...
        const res = await daemonFetch(
          `${args.daemonUrl}/org/create-agent`,
          {
            ...
            body: JSON.stringify({
              ...
              connection: cli.toLowerCase() === 'ssh' ? {
                host: a.host ? String(a.host) : undefined,
                user: a.user ? String(a.user) : undefined,
                port: a.port ? Number(a.port) : undefined,
                identityFile: a.identityFile ? String(a.identityFile) : undefined,
                startupCommand: a.startupCommand ? String(a.startupCommand) : undefined,
              } : cli.toLowerCase() === 'shell' && a.startupCommand ? {
                startupCommand: String(a.startupCommand),
              } : undefined,
            }),
          },
          15_000,
