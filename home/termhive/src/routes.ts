...
export function createRouter(
  ...
  router.post('/projects/:id/agents', (req: Request, res: Response) => {
    const { name, cli, cwd, role, flags, connection } = req.body;
    if (!name || !cli) {
    ...
    const allowed = new Set(['claude', 'codex', 'gemini', 'opencode', 'shell', 'ssh']);
    if (!allowed.has(cli)) { res.status(400).json({ error: 'Unsupported terminal type' }); return; }
    if (cli === 'ssh' && (!connection?.host || !connection?.user)) {
      res.status(400).json({ error: 'SSH host and user are required' });
      return;
    }
    const agentCwd = cwd || projectData.project.cwd;
    const agent = storage.createAgent(req.params.id, name, cli, agentCwd, role, flags, connection);
  ...
  router.get('/snippets', (req: Request, res: Response) => {
    res.json(storage.listSnippets(req.query.projectId as string | undefined));
  });

  router.post('/snippets', (req: Request, res: Response) => {
    const { name, command, category, projectId } = req.body || {};
    if (!String(name || '').trim() || !String(command || '').trim()) {
      res.status(400).json({ error: 'name and command are required' });
      return;
    }
    res.status(201).json(storage.createSnippet({
      name: String(name),
  ...
  router.put('/snippets/:sid', (req: Request, res: Response) => {
    const snippet = storage.updateSnippet(req.params.sid, req.body || {});
    if (!snippet) { res.status(404).json({ error: 'Snippet not found' }); return; }
    res.json(snippet);
  });

  router.delete('/snippets/:sid', (req: Request, res: Response) => {
    if (!storage.deleteSnippet(req.params.sid)) {
