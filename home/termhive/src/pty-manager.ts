...
function getCliCommand(
  ...
  switch (agent.cli) {
    ...
    case 'shell':
      return { cmd: agent.connection?.startupCommand?.trim() || null, args };
    case 'ssh': {
      const host = agent.connection?.host?.trim();
      const user = agent.connection?.user?.trim();
      if (!host || !user) return { cmd: null, args };
      args.push(
        '-F', '/dev/null',
        '-o', 'ServerAliveInterval=30',
        '-o', 'ServerAliveCountMax=3',
        '-o', 'StrictHostKeyChecking=accept-new',
        '-p', String(agent.connection?.port || 22),
      );
      if (agent.connection?.identityFile?.trim()) {
        args.push('-i', expandHome(agent.connection.identityFile.trim()));
      }
      args.push(`${user}@${host}`);
      if (agent.connection?.startupCommand?.trim()) {
        args.push(agent.connection.startupCommand.trim());
      }
      return { cmd: 'ssh', args };
    }
  }
...
export function startAgent(agent: Agent, onStatus: (agentId: string, status: string) => void): boolean {
  ...
  const instructionFiles: Partial<Record<Agent['cli'], string>> = {
    claude: 'CLAUDE.md',
    codex: 'AGENTS.md',
    gemini: 'AGENTS.md',
    opencode: 'AGENTS.md',
  };
  const instructionFile = instructionFiles[agent.cli];
  if (instructionFile) {
    const instrFile = path.join(cwd, instructionFile);
