...
const VALID_CLIS = ['claude', 'codex', 'gemini', 'opencode', 'shell', 'ssh'];
...
export function createAgentDispatch(
  ...
  if (cliNorm === 'ssh' && (!connection?.host?.trim() || !connection?.user?.trim())) {
    return {
      ok: false,
      status: 'error',
      projectName: project.name,
      error: 'SSH agents require connection.host and connection.user.',
    };
  }
