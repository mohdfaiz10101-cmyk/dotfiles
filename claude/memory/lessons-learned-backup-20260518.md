...
- [2026-05-11] [GLM自动] 观察: LiteLLM 4000端口运行但401无API key; Letta 8283响应但health端点返回Not Found可能路径变更; Hub 9800 healthy正常
...
- [2026-05-11] [GLM自动] 观察: op-connection-guard服务timeout失败且日志停在05-04后无更新; 4个user服务failed(marketing-research/architecture-audit/rules-sync/auto-fix); system级health-monitor/resolvconf也failed
- [2026-05-12] [GLM自动] 观察: 4个user service长期failed状态(aggregate-marketing-research/ai-architecture-audit/ai-rules-sync-windows/auto-fix-services)，需清理或修复
- [2026-05-12] [GLM自动] 观察: LiteLLM:4000返回401 auth_error，服务运行但鉴权失败，需检查api_key配置是否被覆盖
- [2026-05-12] [GLM自动] 观察: OP connection-guard日志停在5/4共8天未更新，可能timer已停或被其他机制替代；service-nurse日志含snip残留命令
...
- [2026-05-17] [GLM-5.1] 场景：OpenCode sisyphus tmux session 每5分钟崩溃重启死循环。根因：**opencode-worktree 插件** package.json 缺少 `main`/`module` 入口字段（它是 CLI 工具不是可加载插件），Bun 启动时报 `Cannot find module` → opencode 退出 → sisyphus-guard 重建 tmux → 再崩溃 → 循环。次要原因：**oh-my-openagent** npm 代理配置异常 `proxy.url must be a non-empty string`。修复：从 opencode.json plugin 数组中移除 `opencode-worktree`。预防：安装新插件后 MUST 检查日志确认无 ERROR，sisyphus-guard 应加插件错误检测（连续3次启动失败则暂停重建并通知）。配置：`~/.config/opencode/opencode.json`
- [2026-05-17] [GLM自动] 观察: 4个failed user service长期未恢复——aggregate-marketing-research、ai-rules-sync-windows、hermes-agent、letta-health-guard
- [2026-05-17] [GLM自动] 观察: 4个failed user service——aggregate-marketing-research、ai-rules-sync-windows、hermes-agent、letta-health-guard，均长期未恢复
- [2026-05-17] [GLM自动] 观察: LiteLLM localhost:4000返回401 auth_error，服务运行但未配置认证头；根分区89%超85%警戒线需gc清理
- [2026-05-17] [GLM自动] 观察: LiteLLM localhost:4000返回401 auth_error服务运行但未配置认证头；根分区89%超85%警戒线需gc清理
- [2026-05-17] [GLM自动] 观察: LiteLLM localhost:4000 返回401 auth_error（无api key），服务在运行但未配置认证；根分区89%已超85%警戒线，需nix-collect-garbage清理
- [2026-05-17] [GLM自动] 教训: snip前缀命令实际会执行，多次重试导致lessons-learned.md重复追加3条x3次；后续追加前应先tail检查末行避免重复
...
- [2026-05-17] [GLM自动] 观察: LiteLLM 4000端口在监听但health返回401无api_key错误，需检查.env或启动参数。4个user service失败(aggregate-marketing/ai-rules-sync/mihomo-sync/op-precheck)。/mnt/ai磁盘85%达警戒线。connection-guard日志停在5月4日后无更新。
- [2026-05-17] [GLM自动] 观察: Hub API(9800)、bridge(24801)、opencode-web(8080)、mihomo(7890)均正常监听。Letta 8283未在监听但ai-config-guard日志显示OK，可能端口变更。service-nurse scheduler显示skip状态，缩短间隔至60min。
- [2026-05-17] [GLM自动] 运维: mihomo-sync修复(阈值400KB→1KB)+Docker僵尸容器清理(Exited 127×2)。mihomo-sync现在active，配置17.5KB正常同步。
- [2026-05-17] [GLM自动] 运维: op-tasks.md检查完毕，无未完成高优先级任务。Docker僵尸容器(Exited 127×2)再次清理成功。failed services=0。mihomo-sync已修复运行正常。
- [2026-05-17] [GLM自动] 运维: docker-health-nurse.sh修复—镜像不存在时跳过不记录，避免僵尸容器重复写入op-tasks。Docker Exited(127)容器已清理，nurse下次巡检不会重复报错。
- [2026-05-17] [GLM自动] 运维: docker-health-nurse.sh修复完成—镜像不存在时跳过不记录，避免僵尸容器重复写入op-tasks。Docker Exited(127)容器因数据库锁定无法删除，但不影响运行服务(mihomo/litellm正常)。
- [2026-05-17] [GLM自动] 运维: op-precheck修复(grep-c空值处理)+service-config-guard修复(set-e移除+REPEAT_COUNT默认值)。两服务现在ExecMainStatus=0正常运行。/nix-store 33GB(85%)，nix-collect-garbage释放22.98MiB但显示仍85%(缓存延迟)。
- [2026-05-18] [GLM-5.1] 场景：GPU崩溃(NVIDIA Xid 154)后Docker容器大面积损坏+宿主服务与Docker容器端口冲突循环崩溃。根因链：(1)NVIDIA open-gpu-kernel-modules bug #1134 → BAR1溢出 → GPU崩溃 → Docker containerd overlay/blob损坏 (2)rebuild后宿主systemd服务(chromadb.service/letta.service/postgresql.service)自动启动占端口8000/8283/5432 → Docker容器启动失败 → health-monitor报错 → 循环。修复：(1)全量清理containerd存储重建 `sudo rm -rf docker/containerd docker/rootfs docker/containers docker/network` (2)禁用宿主chromadb.service/letta.service/postgresql.service让Docker版本接管 (3)health-monitor改为端口探测替代容器名匹配 (4)Floorp加--disable-gpu防复发。**关键教训：data-root迁移后必须清空containers/network子目录，否则ghost container残留导致compose持续失败。**
- [2026-05-18] [OP] 用户偏好: 所有任务默认OP自行处理，不再委派CC | 生效范围: op-tasks.md + AGENTS.md + sisyphus agent配置
- [2026-05-18] [OP] 成功记录: crewai-gateway systemd | 调用: systemd --user service + LD_LIBRARY_PATH=/nix/store/...-gcc-15.2.0-lib/lib | 结果: 成功 | 场景: NixOS 上 Python venv 需 gcc libstdc++
