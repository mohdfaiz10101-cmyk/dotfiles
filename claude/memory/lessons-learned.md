- [2026-05-04] [GLM-5.1] 场景：Caddy WS反向代理与ttyd(libwebsockets)不兼容，终端iframe黑屏。根因：Caddy将WebSocket二进制帧解析为HTTP，报`malformed MIME header: missing colon: "\x89\x00"`导致502。修复：iframe直连ttyd端口(7690/7691)绕过Caddy。关键点：(1)`window.location.hostname`在手机上可能≠主机IP（手机Tailscale IP≠主机Tailscale IP），但手机访问7699时hostname就是主机IP（因为7699只跑在主机上）(2)浏览器缓存极顽固——改文件名(index-v2.html)比加Cache-Control header更可靠(3)防火墙需放行直连端口或trustedInterfaces全放行。Caddyfile: `/mnt/ai/apps/launcher/Caddyfile`
