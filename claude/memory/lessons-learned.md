- [2026-05-04] [GLM-5.1] 场景：Caddy WS反向代理与ttyd(libwebsockets)黑屏。**双重修复**：(1) ttyd加`-b /cct`(base-path)，Caddy删除strip_prefix (2) ttyd加`-I`自定义index.html(13KB)替代内置HTML(729KB)。根因：Tailscale relay延迟700ms+，729KB HTML传输中断→broken pipe→502。自定义index缩小56倍解决。验证：Python websockets库测试Caddy代理WS双向通信OK，手机Chrome实测终端正常显示。注意：curl测试WS不可靠（报502），实际浏览器WS正常。文件：ttyd-cct.service/ttyd-claude.service + Caddyfile + index-v2.html
