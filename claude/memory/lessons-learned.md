- [2026-05-04] [GLM-5.1] 场景：Caddy WS反向代理与ttyd(libwebsockets)黑屏。**最终修复**：ttyd加`-b /cct`(base-path)，Caddy删除strip_prefix + 加`encode gzip`(729KB→191KB)，**移除`-I`自定义index.html**（自定义HTML缺少xterm.js/ttyd客户端代码导致终端空白）。验证：Python websockets库测试Caddy代理WS双向通信OK，gzip压缩后191KB即使Tailscale relay也能传输。注意：curl测试WS不可靠（报502），实际浏览器WS正常。文件：ttyd-cct.service/ttyd-claude.service + Caddyfile
