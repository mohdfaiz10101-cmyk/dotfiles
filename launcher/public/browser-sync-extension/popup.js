...
document.getElementById('pushBtn').addEventListener('click', () => {
  chrome.runtime.sendMessage({ action: 'fullSync' });
  showMessage('正在推送数据到其他设备...');
});

document.getElementById('pullBtn').addEventListener('click', () => {
  chrome.runtime.sendMessage({ action: 'pullSync' });
  showMessage('正在从其他设备拉取数据...');
});

document.getElementById('toggleBtn').addEventListener('click', () => {
  chrome.runtime.sendMessage({ action: 'toggleSync' }, (response) => {
...
function showMessage(msg) {
  ...
  setTimeout(() => {
    status.innerHTML = `
      <div>服务器: <span class="connected">192.168.123.1:9978</span></div>
